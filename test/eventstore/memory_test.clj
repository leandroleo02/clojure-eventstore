...
(deftest adding-events-to-the-eventstore
  (let [in-memory (store/in-memory-event-store)]
    (testing "Add event to an aggregation"
      ...
      (let [events (eventstore/get-events in-memory (d/->Stream "orders" 123456))]
        (is (= 1 (count events)))))

    (testing "Add another event to the aggregation"
      (eventstore/add-event in-memory (d/->Stream "orders" 123456) "json 2")
      (let [events (eventstore/get-events in-memory (d/->Stream "orders" 123456))]
        (is (= 2 (count events)))))

    (testing "Add an event to a different id in the same aggregation"
      (eventstore/add-event in-memory (d/->Stream "orders" 654321) "json 3")
      (let [events (eventstore/get-events in-memory (d/->Stream "orders" 654321))]
        (is (= 1 (count events)))))

    (testing "Add an event to a different aggregation"
      (eventstore/add-event in-memory (d/->Stream "account" 123456) "json 4")
      (let [events (eventstore/get-events in-memory (d/->Stream "account" 123456))]
        (is (= 1 (count events)))))))

(deftest retrieving-eventos-from-eventstore
  (let [in-memory (store/in-memory-event-store)]
    (testing "No events added"
      (is (= 0 (count (eventstore/get-events in-memory (d/->Stream "orders" 6543210))))))

    (testing "Two events added"
      (add-n-events in-memory 2 "orders" 1010101)
      (is (= 2 (count (eventstore/get-events in-memory (d/->Stream "orders" 1010101))))))

    (testing "Filter eventos with offset and return the last event"
      (add-n-events in-memory 5 "orders" 3131313)
      (let [events (eventstore/get-events in-memory (d/->Stream "orders" 3131313) 4)]
        (is (= 1 (count events)))))

    (testing "Filter eventos with offset and limit and return the last two events"
      (add-n-events in-memory 5 "orders" 2121212)
      (let [events (eventstore/get-events in-memory (d/->Stream "orders" 2121212) 3 2)]
        (is (= 2 (count events)))))))

(deftest retrieving-aggregations-from-eventstore
  (let [in-memory (store/in-memory-event-store)]
    (testing "No aggregations in the store"
      (let [aggregations (eventstore/get-aggregations in-memory)]
        (is (empty? aggregations))))

    (testing "Two aggregations in the store"
      (eventstore/add-event in-memory (d/->Stream "orders" 123456) "json 1")
      (eventstore/add-event in-memory (d/->Stream "account" 654321) "json 2")
      (let [aggregations (eventstore/get-aggregations in-memory)]
    ...
    (testing "Filtering aggregations result by offset"
      (eventstore/add-event in-memory (d/->Stream "orders" 123456) "json 1")
      (eventstore/add-event in-memory (d/->Stream "account" 654321) "json 2")
      (eventstore/add-event in-memory (d/->Stream "offers" 000001) "json 3")
      (let [aggregations (eventstore/get-aggregations in-memory 2)]
    ...
    (testing "Filtering aggregations result by offset and limit"
      ...
      (eventstore/add-event in-memory (d/->Stream "account" 654321) "json 2")
      (eventstore/add-event in-memory (d/->Stream "offers" 000001) "json 3")
      (eventstore/add-event in-memory (d/->Stream "users" 100000) "json 4")
      (let [aggregations (eventstore/get-aggregations in-memory 1 2)]
        (is (= 2 (count aggregations)))))))

(deftest retrieving-streams-from-eventstore
  (let [in-memory (store/in-memory-event-store)]
    (testing "No streams in the store"
      (let [streams (eventstore/get-streams in-memory "orders")]
        (is (empty? streams))))

    (testing "Two streams in the store"
      (eventstore/add-event in-memory (d/->Stream "orders" 123456) "json 1")
      (eventstore/add-event in-memory (d/->Stream "orders" 654321) "json 2")
      (let [streams (eventstore/get-streams in-memory "orders")]
        (is (= 2 (count streams)))))
    ...
    (testing "Filtering streams result by offset"
      (eventstore/add-event in-memory (d/->Stream "orders" 123456) "json 1")
      (eventstore/add-event in-memory (d/->Stream "orders" 654321) "json 2")
      (eventstore/add-event in-memory (d/->Stream "orders" 100001) "json 3")
      (let [streams (eventstore/get-streams in-memory "orders" 2)]
        (is (= 1 (count streams)))))
    ...
    (testing "Filtering streams result by offset and limit"
      ...
      (eventstore/add-event in-memory (d/->Stream "orders" 654321) "json 2")
      (eventstore/add-event in-memory (d/->Stream "orders" 100001) "json 3")
      (eventstore/add-event in-memory (d/->Stream "orders" 100002) "json 4")
